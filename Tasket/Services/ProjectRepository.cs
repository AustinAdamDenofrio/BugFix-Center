...
namespace Tasket.Services
{
    public class ProjectRepository(IServiceProvider svcProvider, IDbContextFactory<ApplicationDbContext> _dbContextFactory) : IProjectRepository
    {
        ...
        public async Task<IEnumerable<Project>> GetAllAssignedProjectsAsync(int companyId, string userId)
        {
            using ApplicationDbContext context = _dbContextFactory.CreateDbContext();

            IEnumerable<Project> projects = await context.Projects
                                                .Where(p => p.CompanyId == companyId && p.Archived == false && p.Members.Any(m => m.Id == userId))
                                                .Include(p => p.Tickets)
                                                .Include(p => p.Members)
                                                .ToListAsync();
            return projects;
